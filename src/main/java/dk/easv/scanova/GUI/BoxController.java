...
public class BoxController {
    ...
    public void initialize() {
        colBoxLabel.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getLabel()));
        colBoxArchive.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getArchiveName()));
        colBoxProfile.setCellValueFactory(data ->
                new SimpleStringProperty(data.getValue().getProfileName() != null
                        ? data.getValue().getProfileName() : "None"));
        ...
        TableColumn<Box, Void> actionsCol = new TableColumn<>("Actions");
        ...
        actionsCol.setCellFactory(col -> new TableCell<>() {
            ...
            private final HBox   box           = new HBox(6, editBtn, deactivateBtn, reactivateBtn);
            {
                ...
                editBtn.setOnAction(e -> {
                    boxTable.getSelectionModel().select(getIndex());
                    handleEditBox();
                });
        ...
        boxTable.getColumns().add(actionsCol);
    ...
    private void loadArchives() {
        try {
            ObservableList<Archive> archives =
                    FXCollections.observableArrayList(archiveManager.getAllArchives());
    ...
    private void loadBoxes() {
        try {
            ObservableList<Box> boxes =
                    FXCollections.observableArrayList(boxManager.getAllBoxesIncludingInactive());
            boxTable.setItems(boxes);
            if (boxCountBadge != null)
                boxCountBadge.setText(String.valueOf(boxes.stream().filter(Box::isActive).count()));
        } catch (Exception e) {
    ...
    private void handleSaveBox() {
        ...
        try {
            int profileId = selectedProfile != null ? profileDAO.getProfileIdByName(selectedProfile) : -1;

            if (boxBeingEdited == null) {
                if (selectedArchive == null) throw new Exception("Please select an archive.");
                boxManager.createBox(label, selectedArchive.getId(), profileId, getCurrentUserId());
                showFeedback("Box '" + label + "' created!", true);
            } else {
                boxManager.updateBox(boxBeingEdited.getId(), label,
                        selectedArchive != null ? selectedArchive.getId() : boxBeingEdited.getArchiveId(),
    ...
    private void handleEditBox() {
        ...
        if (selected == null) { showFeedback("Please select a box to edit.", false); return; }
        boxBeingEdited = selected;
        labelField.setText(selected.getLabel());
        if (selected.getProfileName() != null) profileComboBox.setValue(selected.getProfileName());
    ...
    private void handleDeleteBox() {
        Box selected = boxTable.getSelectionModel().getSelectedItem();
        if (selected == null) { showFeedback("Please select a box to deactivate.", false); return; }
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
        confirm.setTitle("Deactivate Box");
        confirm.setHeaderText("Deactivate '" + selected.getLabel() + "'?");
        confirm.setContentText("The box will be deactivated and cannot be used for scanning.");
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    boxManager.deleteBox(selected.getId(), getCurrentUserId(), selected.getLabel());
    ...
    private void handleReactivateBox() {
        Box selected = boxTable.getSelectionModel().getSelectedItem();
        if (selected == null) { showFeedback("Please select a box to reactivate.", false); return; }
        if (selected.isActive()) { showFeedback("Box is already active.", false); return; }
        try {
            boxManager.reactivateBox(selected.getId(), getCurrentUserId(), selected.getLabel());
